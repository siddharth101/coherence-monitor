from utils import get_observing_segs, get_times, calc_coherence, run_coherence, get_max_corr, get_frame_files, get_strain_data, get_unsafe_channels, plot_max_corr_chan
...
def run_process(channel_df, time, ifo, strain_data, dur):
    ...
    if len(channel_df)>850:
        ...
        processes = [multiprocessing.Process(target = get_coherence_chan, 
                             args=(channel_df.iloc[i:i+50]['channel'],
                                 time,ifo, ht_data,dur)) for i in range(0,850,50)]
...
# dirs_path =  os.path.join('/home/siddharth.soni/public_html/coherence_monitor/', args.date, 'data','')
# print(dirs_path)
...
#         os.makedirs(path_)
#     plot_max_corr_chan(path=path_, ifo=args.ifo, fft=10) 
